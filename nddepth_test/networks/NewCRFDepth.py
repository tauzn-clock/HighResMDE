...
class NewCRFDepth(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        if version[:-2] == 'base':
            ...
            swin_model = "microsoft/swin-large-patch4-window7-224-in22k"
            embed_dim = 192
            depths = [2, 2, 18, 2]
        ...
        self.backbone = SwinModel.from_pretrained(swin_model)
    ...
    def forward(self, imgs, inv_K, epoch):
        ...
        tmp = self.backbone(imgs, output_hidden_states=True) # DX: Get Features from SWIM backbone
        feats = (tmp["hidden_states"][0].view(B, 192, 120, 160),
                 tmp["hidden_states"][1].view(B, 384, 60, 80),
                 tmp["hidden_states"][2].view(B, 768, 30, 40),
                 tmp["hidden_states"][3].view(B, 1536, 15, 20))
